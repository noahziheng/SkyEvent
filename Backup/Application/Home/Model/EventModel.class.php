...
class EventModel extends Model{
    ...
    {
        $datas = $this->order('status desc,abs( starttime - '.time().')')->where('status != 1')->select();
        foreach ($datas as $k => $data) {
            if (isset($statusflaq) and $statusflaq == false) {
                $this->statusCheck($data['status'],$data['starttime'],$data['endtime'],$data['id']);
            }else{
    ...
    {
    	$datas = $this->order('status desc,abs( starttime - '.time().')')->field('id,title,type,status,author')->select();
    	foreach ($datas as $k => $data) {
