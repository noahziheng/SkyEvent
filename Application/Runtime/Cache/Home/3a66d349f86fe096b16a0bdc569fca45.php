			...
			<table class="table">
				...
				<tbody>
					<?php if(is_array($events)): $i = 0; $__LIST__ = $events;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$event): $mod = ($i % 2 );++$i;?><tr id="tr-<?php echo ($event["id"]); ?>">
							...
							<td><?php if(is_array($event['country'])): $i = 0; $__LIST__ = $event['country'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><img src="<?php echo (ROOT_URL); ?>Public/images/lang/<?php echo ($v); ?>.png">&nbsp;<?php endforeach; endif; else: echo "" ;endif; ?></td>
							<td>
								<button type="button" class="btn btn-default btn-sm">booking管理</button>
								...
								<button type="button" class="btn btn-default btn-sm"><?php echo (L("admin_edit")); ?></button>
								<button type="button" class="btn btn-danger btn-sm"  onclick="confirm('<?php echo (L("admin_del")); ?>',<?php echo ($event["id"]); ?>,'del')"><?php echo (L("admin_del")); ?></button></td>
